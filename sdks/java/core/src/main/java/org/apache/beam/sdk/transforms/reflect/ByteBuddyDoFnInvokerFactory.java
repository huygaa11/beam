...
import org.apache.beam.sdk.transforms.reflect.DoFnSignature.Parameter.OnWindowExpirationContextParameter;
...
public class ByteBuddyDoFnInvokerFactory implements DoFnInvokerFactory {
  ...
  public static final String ON_WINDOW_EXPIRATION_CONTEXT_PARAMETER_METHOD = "onWindowExpirationContext";
  ...
  private static Class<? extends DoFnInvoker<?, ?>> generateInvokerClass(DoFnSignature signature) {
    ...
    DynamicType.Builder<?> builder =
        new ByteBuddy()
            ...
            .intercept(delegateMethodWithExtraParametersOrNoop(
  ...
  /**
   * Implements {@link DoFnInvoker}'s method with extra parameters by delegating to a
   * "target method" of the wrapped {@link DoFn}.
   */
  ...
  static StackManipulation getExtraContextParameter(
    ...
    return parameter.match(
        new Cases<StackManipulation>() {
          ...
          @Override
          public StackManipulation dispatch(OnWindowExpirationContextParameter p) {
            return new StackManipulation.Compound(
                pushDelegate,
                MethodInvocation.invoke(
                    getExtraContextFactoryMethodDescription(
